...
int main(int argc, char **argv) {
    ...
    try {
        auto cloud_service = simulation.add(new wrench::hyperflow::LambdaComputeService(
    ...
    auto wms = simulation.add(
            new wrench::hyperflow::HyperFlowWMS(std::unique_ptr<wrench::StandardJobScheduler>(
                    new wrench::hyperflow::HyperFlowJobScheduler(storage_service)),
    ...
    for (auto const &item : trace) {
        std::cerr << "Task " << item->getContent()->getTask()->getID() << " completed at time " << item->getDate()
                  << " on host " << item->getContent()->getTask()->getExecutionHost() << std::endl;
    }

    simulation.getOutput().dumpUnifiedJSON(workflow, "wrench.json");
